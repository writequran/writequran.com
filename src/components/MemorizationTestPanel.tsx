      ...
      <div className="mb-6 sm:mb-8 rounded-2xl sm:rounded-3xl border border-[#D6C19E]/40 bg-white/90 dark:bg-neutral-900/90 shadow-lg backdrop-blur-sm px-4 py-3 sm:px-6 sm:py-5">
        <div className="flex items-start sm:items-center justify-between gap-4">
          <div className="flex flex-col gap-2 sm:gap-3">
            <h2 className="text-lg sm:text-2xl font-bold text-neutral-800 dark:text-neutral-100 flex flex-wrap items-center gap-1.5 sm:gap-3">
              <span>{n(surahMeta?.number || 0)}. {language === 'ar' ? surahMeta?.name : surahMeta?.englishName}</span>
              <span className="hidden sm:inline text-neutral-300 dark:text-neutral-700 font-normal">|</span>
              <span className="text-[#D6C19E]">{t("ayah")} {n(ayahBlock.ayahNumber)}</span>
            </h2>
            <span className="inline-flex items-center rounded-md bg-neutral-100 dark:bg-neutral-800 px-2 py-1 text-[11px] sm:text-xs font-semibold text-neutral-500 dark:text-neutral-400 w-fit tracking-wide uppercase">
              {typingMode === "word" ? t("word_by_word") : t("letter_by_letter")}
            </span>
          </div>
